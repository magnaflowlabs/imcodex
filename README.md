...
docker build \
  ...
  --build-arg IMCODEX_IMAGE_REVISION=2.2.13 \
...
`v2.2.13` keeps host runtime as the default and further hardens Telegram delivery
...
```text
imcodex 2.2.13 started: config=/srv/imcodex/imcodex.yaml platform=telegram runtime=host-codex groups=1 jobs=1 base=https://api.telegram.org
```
